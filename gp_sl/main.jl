...
@gen function covariance_prior(cur::Int, changepoints::Bool, max_depth::Float64, min_loc::Float64=0., max_loc::Float64=1.)
    node_dist = get_node_dist(cur, changepoints, max_depth)
    node_type = @trace(categorical(node_dist), (cur, :type))

    if node_type == CONSTANT
        value = @trace(uniform_continuous(0, 1), (cur, :value))
        node = Constant(value)
    ...
    elseif node_type == PLUS
        ...
        left_node = @trace(covariance_prior(child1, false, max_depth, min_loc, max_loc))
        right_node = @trace(covariance_prior(child2, false, max_depth, min_loc, max_loc))
        node = Plus(left_node, right_node)
    ...
    elseif node_type == TIMES
        ...
        left_node = @trace(covariance_prior(child1, false, max_depth, min_loc, max_loc))
        right_node = @trace(covariance_prior(child2, false, max_depth, min_loc, max_loc))
        node = Times(left_node, right_node)

    elseif node_type == CHANGEPOINT
        @assert changepoints
        location = @trace(uniform_continuous(min_loc, max_loc), (cur, :location))
        child1 = Gen.get_child(cur, 1, max_branch)
        child2 = Gen.get_child(cur, 2, max_branch)
        left_node = @trace(covariance_prior(child1, false, max_depth, min_loc, max_loc))
        right_node = @trace(covariance_prior(child2, false, max_depth, min_loc, max_loc))
        dim = @trace(categorical(dim_dist), (cur, :dim))
        node = ChangePoint(left_node, right_node, location, .001, dim)
...
@gen function model(input::Vector{Vector{Float64}}, changepoints::Bool, max_depth::Float64, min_loc::Float64, max_loc::Float64)
    n = length(input[1])
    covariance_fn_x = @trace(covariance_prior(1, changepoints, max_depth, min_loc, max_loc), :tree_x)
    covariance_fn_y = @trace(covariance_prior(1, changepoints, max_depth, min_loc, max_loc), :tree_y)
...
@gen function subtree_proposal(prev_trace, idx, biased::Bool)
    (input, changepoints, max_depth, min_loc, max_loc) = get_args(prev_trace)
    # Choose a random node.
    pick_node = biased ? pick_random_node_biased : pick_random_node
    root = prev_trace[][idx]
    (subtree_node, subtree_idx, subtree_depth) =
        @trace(pick_node(root, 1, 0), :choose_subtree_root)
    # Simulate fresh subtree at selected random.
    cps = begin
        if !changepoints
            false
    ...
    subtree_propsed = @trace(covariance_prior(subtree_idx, cps, max_depth, min_loc, max_loc), :subtree)
    # Return the subtree.
    return (subtree_idx, subtree_depth, subtree_propsed)
end

function subtree_involution_x(
        trace,
        forward_choices::ChoiceMap,
        forward_retval::Tuple,
        proposal_args::Tuple)
    # Populate backward assignment with choice of root.
    (subtree_idx, subtree_depth, new_subtree_node) = forward_retval
    backward_choices = choicemap()
    set_submap!(
        backward_choices, :choose_subtree_root => :recurse_left,
        get_submap(forward_choices, :choose_subtree_root => :recurse_left))
    for depth=0:subtree_depth-1
        backward_choices[:choose_subtree_root => :done => depth] = false
    end
    if !isa(new_subtree_node, LeafNode)
        backward_choices[:choose_subtree_root => :done => subtree_depth] = true
    end

    # Populate constraints with proposed subtree
    # then obtain new trace and discard, which contains previous subtree.
    model_args = get_args(trace)
    constraints = choicemap()
    set_submap!(constraints, :tree_x, get_submap(forward_choices, :subtree))
    (new_trace, weight, retdiff, discard) = update(
        trace, model_args, (NoChange(),), constraints)

    # Populate backward assignment with the previous subtree.
    set_submap!(backward_choices, :subtree, get_submap(discard, :tree_x))
    return (new_trace, backward_choices, weight)
end


function subtree_involution_y(
    trace,
    forward_choices::ChoiceMap,
    forward_retval::Tuple,
    proposal_args::Tuple)
    ...
    model_args = get_args(trace)
    constraints = choicemap()
    set_submap!(constraints, :tree_y, get_submap(forward_choices, :subtree))
    (new_trace, weight, retdiff, discard) = update(
        trace, model_args, (NoChange(),), constraints)

    # Populate backward assignment with the previous subtree.
    set_submap!(backward_choices, :subtree, get_submap(discard, :tree_y))
    return (new_trace, backward_choices, weight)
end
...
function compute_particle_predictions_no_trace(
    cov_fn_x,
    cov_fn_y,
    noise_x,
    noise_y,
...
function experiment_smc(
        ...
        seed::Int=1,
        save::Bool=true)
    max_loc = maximum([maximum(input[d]) for d in 1:length(input)])
    min_loc = minimum([minimum(input[d]) for d in 1:length(input)])
    ...
    for t=t_start:t_step:length(input[1])-1
        ...
        Threads.@threads for i=1:n_particles
            ...
            for iter=1:n_mcmc
                # MH move on the subtree.
                tr, = metropolis_hastings(
                    tr, subtree_proposal, (1, biased,), subtree_involution_x)
                tr, = metropolis_hastings(
                    tr, subtree_proposal, (2, biased,), subtree_involution_y)
